import type { TemperatureElement } from "../types";

export const AIRFRYER_TIMES: TemperatureElement[] = [
  { name: "Escalivada", time: 30, temperature: 200 },
  { name: "Ou dur", time: 15, temperature: 150 },
  { name: "Patata petita", time: 30, temperature: 200 },
  { name: "Patata gran", time: 40, temperature: 200 },
  { name: "Patates fregides (1a fase)", time: 20, temperature: 120 },
  { name: "Patates fregides (2a fase)", time: 10, temperature: 180 },
  { name: "Pebrot fregit", time: 14, temperature: 180 },
  { name: "Pebrots del padrón", time: 8, temperature: 180 },
  { name: "Pollastre", time: 20, temperature: 180 },
  { name: "Seitons", time: 8, temperature: 180 },
  { name: "Verdures rostides", time: 26, temperature: 175 },
];
